...
$sql2 = "select sum(b.mode * b.beiShu * b.actionNum) betAmount, sum(b.bonus) zjAmount from ssc_members u left join ssc_bets b on b.isDelete=0 and u.uid=b.uid $betTimeWhere $userWhere3";
...
<table class="tablesorter" cellspacing="0">
	...
	<thead>
		<tr>
			...
			<td>中奖总额</td>
			<td>总返点</td>
			<td title="包括充值佣金，注册佣金，亏损佣金，消费佣金，签到">佣金及活动</td>
			<td>充值</td>
			<td>提现</td>
			<td>余额</td>
			<td>团队盈亏</td>
...
if ($list['data']) {
	foreach ($list['data'] as $var) {
		if ($var['username'] != '没有下级了') {
			...
			$pId = $var['uid'];
			$var['teamwin'] = $this->getValue("select sum(l.coin) fandianAll from {$this->prename}coin_log l, ssc_members u where l.liqType in(2,3) and l.uid=u.uid and concat(',', u.parents, ',') like '%,$pId,%' $fanDiaTimeWhere") + $this->getValue("select sum(b.bonus-b.mode * b.beiShu * b.actionNum) betZjAmount from ssc_members u ,ssc_bets b where b.isDelete=0 and u.uid=b.uid and concat(',', u.parents, ',') like '%,$pId,%' $betTimeWhere");
		}

...
class CImage
{
    ...
    public function writeChar($code)
    {
    ...
    public function writeArc($i)
    {
        $image = $cx = $cy = $width = $height = $start = $end = $color = '';
        while ($i--) {
            $image = $this->img;
            $cx = rand(1, $this->width);
            $cy = rand(10, $this->heigth);
            $width = rand(20, $this->width);
            $height = rand(1, $this->heigth);
            $start = rand(1, 360);
            $end = 300;
            $red = rand(0, 120);
            $green = rand(0, 120);
    ...
    public function printimg($type)
    {
        header("content-type:image/$type");
        $len = strlen($this->chars);
        $code = '';
        for ($i = 0; $i < $this->charlen; $i++) {
            $code .= $this->chars{mt_rand(0, $len - 1)};
        }

...
class AdminBase extends Object
{
    ...
    public function getUser($uid = null)
    {
        ...
        if (is_int($uid)) {
            return $this->getRow("select * from {$this->prename}members where uid=$uid");
        }

        if (is_string($uid)) {
            return $this->getRow("select * from {$this->prename}members where username=?", $uid);
        }
    ...
    {
        if ($user = unserialize($_SESSION[$this->adminSessionName])) {
            return $user;
        }
    ...
    {
        if (!$date) {
            $date = strtotime(date("Y-m-d", $this->time));
        }

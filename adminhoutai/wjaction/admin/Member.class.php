...
class Member extends AdminBase {
	...
	public final function added($passwordEncrypt = false) {
		...
		if (!isset($update['fanDian'])) {
			throw new Exception('返点不能为空');
		}

		if (!ctype_digit($update['qq'])) {
			throw new Exception('QQ包含非法字符');
		}

		if (strlen($update['qq']) < 4 || strlen($update['qq']) > 13) {
		...
		if (isset($update['password']) && !$passwordEncrypt) {
			$update['password'] = md5($update['password']);
		}

		if (!isset($update['nickname']) || !$update['nickname']) {
	...
	public final function deleteed() {
		...
		if ($son['teamNum'] - 1 > 0) {
			throw new Exception('团队还有成员' . $son['teamNum'] . '人，团队资金' . $son['teamCoin'] . '元,团队冻结' . $son['teamFcoin'] . '元');
		}

		if (floatval($teamCoin) != floatval($son['teamCoin'])) {
			throw new Exception('团队资金刚有变动' . (floatval($son['teamFcoin']) - floatval($teamFcoin)) . '元，请确认后再删除');
		}

		if (floatval($teamFcoin) != floatval($son['teamFcoin'])) {
			throw new Exception('团队冻结资金刚有变动' . (floatval($son['teamFcoin']) - floatval($teamFcoin)) . '元，请确认后再删除');
		}

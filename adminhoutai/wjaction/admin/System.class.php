...
class System extends AdminBase {
	...
	public final function updateSettings() {
		...
		if (!ctype_digit($para['czzs']) || $para['czzs'] > 100 || $para['czzs'] < 0) {
			throw new Exception('请正确输入充值赠送百分比!');
		}

		if (!ctype_digit($para['cashMinAmount']) || $para['cashMinAmount'] > 100 || $para['cashMinAmount'] < 0) {
			throw new Exception('请正确输入消费百分比!');
		}

		if (!ctype_digit($para['betMaxCount'])) {
			throw new Exception('请正确设置最大注数!');
		}

		if (!ctype_digit($para['betMaxZjAmount'])) {
			throw new Exception('请正确设置最大中奖金额!');
		}

		if (!ctype_digit($para['rechargeMin'])) {
			throw new Exception('请正确设置最低充值金额!');
		}

		if (!ctype_digit($para['rechargeMax'])) {
			throw new Exception('请正确设置最高充值金额!');
		}

		if (!ctype_digit($para['rechargeMin1'])) {
			throw new Exception('请正确设置支付宝/财付通最低充值金额!');
		}

		if (!ctype_digit($para['rechargeMax1'])) {
			throw new Exception('请正确设置支付宝/财付通最高充值金额!');
		}

		if (!ctype_digit($para['cashMin'])) {
			throw new Exception('请正确设置最低提现金额!');
		}

		if (!ctype_digit($para['cashMax'])) {
			throw new Exception('请正确设置最高提现金额!');
		}

		if (!ctype_digit($para['cashMin1'])) {
			throw new Exception('请正确设置支付宝/财付通最低提现金额!');
		}

		if (!ctype_digit($para['cashMax1'])) {
			throw new Exception('请正确设置支付宝/财付通最高提现金额!');
		}

		if (!ctype_digit($para['clearMemberCoin'])) {
			throw new Exception('请正确设置清理账号规则金额!');
		}

		if (!ctype_digit($para['clearMemberDate'])) {
			throw new Exception('请正确设置清理账号规则时间!');
		}

		if (!ctype_digit($para['huoDongRegister'])) {
			throw new Exception('请正确设置绑定工行赠送金额!');
		}

		if (!ctype_digit($para['huoDongSign'])) {
			throw new Exception('请正确设置每天签到赠送金额!');
		}

		if (!ctype_digit($para['rechargeCommissionAmount'])) {
			throw new Exception('请正确设置充值佣金金额!');
		}

		if (!ctype_digit($para['rechargeCommission'])) {
			throw new Exception('请正确设置充值佣金上家赠送金额!');
		}

		if (!ctype_digit($para['rechargeCommission2'])) {
			throw new Exception('请正确设置充值佣金上上家赠送金额!');
		}

		if (!ctype_digit($para['conCommissionBase'])) {
			throw new Exception('请正确设置消费佣金金额!');
		}

		if (!ctype_digit($para['conCommissionParentAmount'])) {
			throw new Exception('请正确设置消费佣金上家赠送金额!');
		}

		if (!ctype_digit($para['conCommissionParentAmount2'])) {
			throw new Exception('请正确设置消费佣金上上家赠送金额!');
		}

		if (!ctype_digit($para['LiRunLv'])) {
			throw new Exception('请正确设置系统彩利率!');
		}

		if ($para['LiRunLv'] > 10) {
			throw new Exception('最大利率过高不容易出号，请不要超过10%!');
		}

...
//, $actionFlag,
	 ...
	 * 充值操作
	 */
	public final function rechargeAction($uid, $amount, $uIN, $remarks) {
		if ($uIN == 1) {
			$uid = intval($uid);
			if ($uid <= 0) {
		...
		try {
			...
			$data['info'] = '管理员"' . $this->user['username'] . '"备注:' . $remarks;
			$data['state'] = 9;
	...
	public final function rechargeHandle() {
		...
		if ($data['state']) {
			throw new Exception('充值已经到帐，请不要重复确认');
		}

		if ($data['isDelete']) {
			throw new Exception('充值已经被删除');
		}
	...
	public final function betinfoUpdateed() {
		...
		if ($bet['lotteryNo']) {
			throw new Exception('已开奖过，不能再修改投注信息');
		}

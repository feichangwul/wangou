    ...
    <?php if ($data['data']) foreach ($data['data'] as $var) { ?>
        ...
        <tr>
            ...
            <?php if ($this->user['uid'] != $var['uid'] && $var['parentId'] == $this->user['uid']) { ?>
                <td><a href="/index.php/team/userUpdate/<?= $var['uid'] ?>" style="color:#333;display:none;"
                       target="modal" width="420" title="修改用户" modal="true"
                       button="确定:dataAddCode|取消:defaultCloseModal">修改</a>&nbsp;&nbsp;
                    <?php if ($this->settings['recharge'] == 1) { ?>
                        <a href="/index.php/team/userUpdate2/<?= $var['uid'] ?>" style="color:#333;display:none;"
                           target="modal" width="420" title="给下级充值" modal="true"
                           button="确定:dataAddCode|取消:defaultCloseModal">充值</a>&nbsp;&nbsp;
                    <?php } ?>
                    <a class="caozuo" style="margin-right:14px"
                       href="/index.php/team/searchMember?type=2&uid=<?= $var['uid'] ?>">查看下级</a></td>
            <?php } else { ?>
                <td><a class="caozuo" style="margin-right:14px"
                       href="/index.php/team/searchMember?type=2&uid=<?= $var['uid'] ?>">查看下级</a></td>
            <?php } ?>
        </tr>
    <?php } ?>

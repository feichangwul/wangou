...
<head>
    ...
    <script type="text/javascript">
        $(function () {
            ...
            $('.example2').click(function () {
                ...
                if (src) $('<div>').append($('<img>', {src: src, width: '640px', height: '480px'})).dialog({
                    width: 630,
                    height: 500,
                    title: '充值演示'
                });
    ...
    <script type="text/javascript">
        $(function () {
            $('.example2').click(function () {
                var src = '/' + $(this).attr('rel');
                if (src) $('<img>', {src: src}).css({width: '640px', height: '480px'}).dialog({
                    width: 660,
                    height: 500,
                    title: '充值演示'
                });
                ...
                if (isset($banks) && !empty($banks)) {
                    ...
                    <table width="100%" border="0" cellspacing="1" cellpadding="4" class='table_b'>
                        ...
                        <tr height=25 class='table_b_tr_b'>
                            <td align="center" class="copys" height="80">
                                <div style="display:inline;color:#FF0000">自动充值须知：
                                    <div>
                            </td>
                            <td align="left">
                                <p>每天的充值处理时间为：<b style="display:inline;color:#FF0000;">早上 &nbsp 8:30 &nbsp 至 次日凌晨 &nbsp
                                        02:30</b></p>
                                <p></p>
                                <p>充值后，<b style="display:inline;color:#FF0000;">请手动刷新&nbsp&nbsp</b>你的余额及查看相关账变信息，若超过5分钟未上分，请于客服联系
                                </p>
                                <p></p>
                                ...
                                if ($banks) {
                                    foreach ($banks as $bank) {
                                        ...
                                        <div class="bankchoice">
                                            <label><input value="<?= $bank['id'] ?>" type="radio" name="mBankId"
                                                          data-bank='<?= json_encode($bank) ?>'
                                                          style="width:auto;"/><span
                                                        style="background:url(/<?= $bank['logo'] ?>);"></span></label>
                                        </div>
                                    <?php }
                                }
                                ?>
                        ...
                        <tr height=25 class='table_b_tr_b' id="display-dom">
                            ...
                            <td align="left"><img id="bank-type-icon" class="bankimg" src="" title=""/>
                                <div class="clear"></div>
                            </td>
                        </tr>
                        <tr height=25 class='table_b_tr_b'>
                            <td align="center" class="copys">充值金额：</td>
                            <td align="left"><input name="amount" id="ContentPlaceHolder1_txtMoney"
                                                    min="<?= $set['rechargeMin'] ?>" max="<?= $set['rechargeMax'] ?>"
                                                    min1="<?= $set['rechargeMin1'] ?>"
                                                    max1="<?= $set['rechargeMax1'] ?>" value=""
                                                    onkeyup="showPaymentFee();"/>
                                <div style="display:inline;" class="spn12">( 单笔充值限额 最低： <b
                                            style="color:#FF0000"><?= $set['rechargeMin'] ?></b> 元，最高： <b
                                            style="color:#FF0000"><?= $set['rechargeMax'] ?></b> 元 )
                                </div>
                            </td>
                        </tr>
                        <tr height=25 class='table_b_tr_b'>
                            <td align="center" class="copys">充值金额(大写)：</td>
                            <td align="left"><strong style="color:#FF0000;margin-left:10px" id="chineseMoney"></strong>
                            </td>
                        </tr>
                        <tr height=25 class='table_b_tr_b'>
                            <td align="center" class="copys">验证码：</td>
                            <td align="left"><input name="vcode" type="text"
                                                    style="ime-mode: disabled; width: 75px;"/><b class="yzmNum"><img
                                            width="58" height="24" border="0" style="cursor:pointer;margin-bottom:0px;"
                                            id="vcode" alt="看不清？点击更换" align="absmiddle"
                                            src="/index.php/user/vcode/<?= $this->time ?>" title="看不清楚，换一张图片"
                                            onclick="this.src='/index.php/user/vcode/'+(new Date()).getTime()"/></b>
                            </td>
                        </tr>
                        <tr height=25 class='table_b_tr_b'>
                            <td align="center" style="font-weight:bold;"></td>
                            <td align="left"><input type="button" id='put_button_pass' class="btn darwingbtn"
                                                    value="下一步" onclick="$(this).closest('form').submit()">
